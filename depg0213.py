...
class DEPG0213:
    ...
    def __init__(self, spi, cs, dc, rst, busy, vext=None):
        ...
        # Landscape framebuffer — user draws here
        # 1 = white, 0 = black (matches e-ink convention)
        self._stride = (self.WIDTH + 7) // 8  # 32 bytes/row
        self._buf = bytearray(self._stride * self.HEIGHT)
        self.fb = framebuf.FrameBuffer(
            self._buf, self.WIDTH, self.HEIGHT, framebuf.MONO_HLSB
        )
    ...
    def text(self, s, x, y, c=0):
        """Draw text at (x,y). Default: black text."""
